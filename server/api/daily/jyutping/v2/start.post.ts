// server/api/daily/jyutping/start.post.ts
...
export default defineEventHandler(async (event) => {
  ...
  if (
    ...
    throw createError({
      statusCode: 400,
      statusMessage: "Invalid totalQuestions",
    });
  }
    ...
    const upsert = await client.query(
      `
        ...
        returning
          ...
          answered_count
      `,
      [userId, sessionDate, mode, totalQuestions],
    );
    ...
    const updated = await client.query(
      `
        ...
        returning
          ...
          answered_count
      `,
      [userId, sessionDate, mode, wordIds, totalQuestions],
    );
