...
export default defineEventHandler(async (event) => {
    ...
    `
      select
        ...
        coalesce((
          select sum(delta)
          from (
            select coalesce(q.applied_total_delta, q.total_delta) as delta
            ...
            select coalesce(j.applied_total_delta, j.total_delta) as delta
            from xp_jyutping_events j
            where j.user_id = $1
              and j.processed = true
  ...
  return {
    total_xp: Number(rows[0]?.total_xp ?? 0),
    words_maxed: Number(rows[0]?.words_maxed ?? 0),
    words_seen: Number(rows[0]?.words_seen ?? 0),
    total_correct: Number(rows[0]?.total_correct ?? 0),
    total_wrong: Number(rows[0]?.total_wrong ?? 0),
    xp_this_week: Number(rows[0]?.xp_this_week ?? 0),
  };
});