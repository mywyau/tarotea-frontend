...
function buildWordOutcomeMap(rawAnswers: unknown): Map<string, boolean> {
  ...
  for (const item of rawAnswers) {
    ...
    // Keep first outcome for each word in the batch.
    if (!map.has(wordId)) {
...
export default defineEventHandler(async (event) => {
  ...
  try {
    // Fast idempotency path: if this attempt already exists, return
    // the canonical stored result and re-publish safely.
    ...
    await db.query(
      ...
      [
        ...
        userId,
        mode,
        attemptId, // keep legacy column populated for now
    ...
    const canonical = await getExistingEvent(userId, attemptId);

    if (!canonical) {
      throw new Error("Finalize event was not persisted");
    }

    // Publish only after the event is durably stored.
    // If this publish fails, let the client retry the same attemptId.
    await enqueueFinalizeJob(event, { attemptId, userId });

    return {
      quiz: {
        mode: canonical.mode,
        correctCount: canonical.correct_count,
        totalQuestions: canonical.total_questions,
        xpEarned: canonical.total_delta,
      },
      attemptId,
      queued: true,
    };
