...
const fallbackTopicTitle = computed(() =>
...
const animatedCompletedSentences = ref(0)
...
const completedSentencesCount = computed(() =>
  sessionResult.value?.correctCount ?? 0
)

const totalSentencesCount = computed(() =>
...
const hintFreeCount = computed(() =>
  Math.max(0, completedSentencesCount.value - hintsUsedCount.value)
)

const hintUsageRatio = computed(() => {
  if (!completedSentencesCount.value) return 0
  return hintsUsedCount.value / completedSentencesCount.value
})
...
function resetCompletionAnimations() {
  animatedCompletedSentences.value = 0
...
async function finalizeBatch() {
  ...
  try {
    ...
    console.info('Topic sentence dojo finalized', {
      sessionKey: activeSessionKey.value,
      queued: res.queued,
      deduped: res.deduped,
    })
  } catch (err) {
    console.error('Topic sentence dojo finalize failed', err)
...
watch(
  ...
  (visible) => {
    ...
    if (!completionAnimated.value) {
      ...
      animateCount(animatedCompletedSentences, completedSentencesCount.value, 900)
...
<template>
  <main class="dojo-training-page mx-auto max-w-4xl px-6 py-12">
    ...
    <header class="dojo-training-header space-y-4">
      <TypewriterTitleBlock :heading-text="title || `Sentence Dojo - ${fallbackTopicTitle}`"
        ...
        <div v-if="showTraining" class="space-y-5">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              <AudioButton :key="`audio-${selectedVoice}-${current?.sentenceId}`" :src="currentAudioSrc" :playback-rate="playbackRate" />
        ...
        <transition name="fade-scale" mode="out-in">
          ...
          <div v-else-if="showResults" key="results" class="space-y-6">
            <div class="stat-card hero-card" :class="resultHeroClass">
              ...
              <p class="hero-subtext">
                {{ completedSentencesCount }} / {{ totalSentencesCount }} sentences completed
              </p>
            </div>
