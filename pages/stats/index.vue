...
function animateStat(index: number, endValue: number, duration = 1000) {
  const startValue = 0
  ...
  function update(now: number) {
    ...
    animatedStats.value[index] = Math.round(
    ...
    if (progress < 1) {
      ...
      animatedStats.value[index] = endValue
    }
  }
...
onMounted(async () => {
  try {
    ...
    const res = await $fetch<UserStats>('/api/user/stats', {
      method: 'GET',
      headers: {
    ...
    requestAnimationFrame(() => {
      stats.value.forEach((stat, index) => {
        animateStat(index, stat.value, 900 + index * 120)
      })
    })
  } catch (err) {
