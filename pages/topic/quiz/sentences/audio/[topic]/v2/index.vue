...
const { data } = await useAsyncData(
  () => `topic-sentences-audio-${slug.value}`,
  async () => {
    const token = await getAccessToken()

    return $fetch<TopicSentenceData>(
      `/api/sentences/${slug.value}/rotate-topics`,
      {
        headers: {
...
watch(
  ...
  async (qs) => {
    ...
    try {
      ...
      console.error('Failed to load topic sentence audio quiz word progress', err)
      wordProgressMap.value = {}
      currentXp.value = 0
      currentStreak.value = 0
    }
        ...
        <div class="space-y-6">
          <div class="min-h-[90px] flex items-center justify-center">
            <div class="w-full text-center">
              <div
                class="inline-block text-left space-y-2"
          ...
          <div class="grid grid-cols-1 gap-4 w-full">
            <button
              ...
              :class="[
                !answered && 'hover:-translate-y-1 hover:shadow-lg hover:brightness-110',
