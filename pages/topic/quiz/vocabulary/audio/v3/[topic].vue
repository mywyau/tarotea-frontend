...
const { data } = await useFetch<TopicLevelData>(
...
const wordsForLevel = computed<TopicWord[]>(() => {
...
async function finalizeAudioQuiz() {
  ...
  try {
    ...
    totalXpEarned.value = res?.quiz?.xpEarned ?? 0
  } catch (err) {
...
watch(
  ...
  async (qs) => {
    ...
    if (!qs.length || !isLoggedIn.value) return
